...
program
  ...
  .option('-d, --directory <path/to/directory>', 'direcoty path where to find the metadata files.')
  .parse(process.argv);
...
if (program.directory) {
  ...
  if (fs.existsSync(path.join(__dirname, '..', `tmp/${what}.js`))) {
    filePath = path.join(__dirname, '..', `tmp/${what}.js`);
  } else {
